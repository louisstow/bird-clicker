...
var BuyableBirdView = BirdView.extend({
  ...
  template: _.template('<div class="bird <%= (eggs > model.cost) || model.shown ? "" : "hidden"  %> <%= eggs > model.cost ? "" : "disabled"  %>">' +
    ...
    '<div class="info"><strong><%- model.name %></strong> <small><%= model.rewardPerTick.formatNumber() %> eggs per second</small><p><%- model.description %></p></div>' + 
    '<div class="stats">' +
      '<span class="owned"><%- model.numberOwned.formatNumber() %> owned</span>' +
      '<span class="cost"><%- model.cost.formatNumber() %> eggs</span>' +
    '</div>' + 
